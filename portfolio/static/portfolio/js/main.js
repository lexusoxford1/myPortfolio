// Initialize AOS animations
  AOS.init({
    duration: 1200,
    easing: 'ease-in-out',
    once: false, // allows re-animate on scroll up & down
    mirror: true // follow scroll up/down
  });
...
if (percent >= 100) {
    ...
    // Fade out loader
    loadingScreen.classList.add('opacity-0', 'transition', 'duration-700');
    setTimeout(() => {
        loadingScreen.style.display = 'none';
        mainContent.classList.remove('hidden');

        // Refresh AOS so animations trigger on newly visible elements
        AOS.refresh();
    }, 700);
}
